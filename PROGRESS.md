...
Ran by glm-5.3-flash:cloud in Claude Code (tmux). Four jobs, all closed.

- **Job 1 (retire dead tools):** call-graph BFS from produce_v2.py +
  pod_build.py found 26 of 64 code files reachable. 25 retired (git 3d31f0e;
  tarball + manifest archived to b2:mendymax-archive/soccer-channel/2026-09-23/).
  12 hand-run tools kept with justification; census in TOOLS.md. parse_feed
  retired (ESPN is the fact source). Test suite rewritten against the live
  pipeline (12/12 pass). Stale references in kept tools fixed. Orphan
  tools/yolov8s.pt duplicate deleted (canonical ~/yolov8s.pt intact).
- **Job 2 (momentum labels):** the minutes-13/23 goal labels collided and the
  title clipped at the figure top. Fix: two-lane stagger + axes headroom
  (boards_2d.py draw_momentum). Verified by frames viewed (before
  boards_job2/mom_before_last.png, after mom_after_last.png, and in the
  re-assembled final at t=122.5).
- **Job 3 (four-section freeze):** the outro board popped chips at separate
  thresholds, so the final segment read as four frozen states. Fix:
  one-composition rise (draw_outro); a mid-fix subtitle/chip crossing was
  caught and removed (unified rise offset). Re-assembled on Modal; verified
  by frames viewed at t=132/137/140 of the new final.
- **Job 4 (canonical sync):** project CLAUDE.md replaced with the owner-
  delivered file; global ~/.claude/CLAUDE.md rewritten to machine-wide truth
  only; STATUS/CONTEXT/TOOLS/GAPS/README/ARCHITECTURE rewritten against the
  code; DECISIONS log appended (flash era + brief 03); EPISODE_SPEC banner
  (under review, historical Gemini refs flagged); invented architecture
  deleted (EP001_MOMENTS, EP001_SCRIPT_v2, HANDOVER_TO_FLASH, the two
  moments workflow scripts, thesis-frame template placeholder,
  falsifiability rule); block-image-read.sh hook deleted + unregistered
  (was a no-op); produce_v2 stale renderer comment + dead tactical_path
  plumbing removed.
